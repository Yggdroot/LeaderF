...
class LfOrderedDict(OrderedDict):
    ...
    def insert_ordered_update(self, key, value):
        """
        if is insert, return True, otherwise return False
        """
        if not self:
            self[key] = value
            return True
...
class TreeView(GitCommandView):
    ...
    def _setChangedFilesNum(self, immediate=False):
        ...
        try:
            if immediate == False and self._content_buffer is not None:
                ...
                self._buffer[self.startLine() - 3] = re.sub(
                        r'(?:\s*\(\d+\))?:$',
                        ' ({}):'.format(num),
                        title
                        )
    ...
    def _insertOrUpdateFileNode(self, path, diff_output):
        *directories, file = path.split("/")
        ...
        insert = tree_node.files.insert_ordered_update(file, source)
        if insert == True:
            self._insertFileList(source)
    ...
    def createFromScratch(self, diff_output):
        ...
        title = self._cmd.getTitle()
        if title is not None:
            content.append(title[:-1] + " (1):")

        content.append(shrinkUser(self._project_root))
    ...
    def _insertFileList(self, source):
        file_path = lfGetFilePath(source)
        index = Bisect.bisect_left(self._file_path_list[self._cur_parent], file_path)
        self._file_path_list[self._cur_parent].insert(index, file_path)
        icon = webDevIconsGetFileTypeSymbol(file_path) if self._show_icon else ""
        self._file_list[self._cur_parent].insert(index, self.buildFileLine(source, icon))
    ...
    def _removeFiles(self, target_path):
        ...
        if "/" in target_path:
            ...
            if is_empty:
                del structure[index]
                del self._trees[self._cur_parent].dirs[first_dir_name]
                self._buffer.options['modifiable'] = True
                if len(structure) == 0:
                    del self._buffer[line_num - 4: line_num]
                else:
            ...
            del structure[index]
            del self._trees[self._cur_parent].files[target_path]
            self._buffer.options['modifiable'] = True
            if len(structure) == 0:
                del self._buffer[line_num - 4: line_num]
            else:
...
class DiffViewPanel(Panel):
    ...
    def configBuffer(self, win_id, k, source, right_winid, **kwargs):
        ...
        lfCmd("""call win_execute(%d, "let b:lf_git_file_path = '%s'")"""
...
class NavigationPanel(Panel):
    ...
    def open(self):
        ...
        current_file_path = os.path.relpath(lfEval("b:lf_git_file_path"), self._project_root)
    ...
    def moveFileBetweenTrees(self, current_tree_view, title, target_path, file_info, focus):
        ...
        if title == "Unstaged Changes:":
            ...
            git_cmd = "git diff --cached --diff-algorithm={} --raw -C --numstat --no-abbrev -- {} {}".format(
                    self._diff_algorithm,
                    shlex.quote(orig_file_path) if orig_file_path else "",
                    shlex.quote(target_path)
                    )
            outputs = ParallelExecutor.run(git_cmd, directory=self._project_root)
        else:
...
class GitBlameExplManager(GitExplManager):
    ...
    def startInlineBlame(self, tmp_file_name):
        if vim.current.buffer.number  in self._blame_infos:
            return
        ...
        cur_buf_number = vim.current.buffer.number
        reader_thread = threading.Thread(target=self._readBlameInfo, args=(cur_buf_number, git_cmd, lfEval("&encoding")))
        reader_thread.daemon = True
        reader_thread.start()

        if self._blame_timer_id is not None:
            lfCmd("call timer_stop(%s)" % self._blame_timer_id)
        self._blame_timer_id = lfEval("timer_start(50, function('leaderf#Git#InlineBlame', [%d, %d]), {'repeat': -1})"
                                      % (id(self), cur_buf_number))

        lfCmd("let g:Lf_git_inline_blame_enabled = 1")

    def _readBlameInfo(self, buf_number, git_cmd, encoding):
        try:
            executor = AsyncExecutor(True)
            ...
            self._blame_info_list[buf_number] = list(content)
        except Exception:
            pass
            # traceback.print_exc()
            # traceback.print_stack()

    def inlineBlame(self, buf_number):
        if vim.current.buffer.number != buf_number:
            if self._blame_timer_id is not None:
                lfCmd("call timer_stop(%s)" % self._blame_timer_id)
                self._blame_timer_id = None
            return

        if buf_number not in self._blame_info_list:
            return
        ...
        if len(self._blame_info_list[buf_number]) == 0:
            return

        blame_list = iter(self._blame_info_list[buf_number])
        i = 0
        ...
        del self._blame_info_list[buf_number]
