...
class GitExplManager(Manager):
    ...
    def setArguments(self, arguments):
        ...
        self._has_column = "--column" in lfEval("get(g:, 'Lf_GitConfig', [])") or "--column" in self._arguments
    ...
    def _beforeExit(self):
        ...
        reg = lfEval("get(g:, 'Lf_GitStorePattern', '')")
        if reg == '':
            return
    ...
    def _createPopupPreview(self, title, source, line_num, jump_cmd=''):
        ...
        if (super(GitExplManager, self)._createPopupPreview(title, source, line_num, jump_cmd)
            and lfEval("get(g:, 'Lf_GitHighlightInPreview', 1)") == '1'):
    ...
    def replace(self):
        ...
        try:
            if not self._getInstance().buffer.options["modifiable"]:
                ...
                lfCmd("augroup Lf_Git_ReplaceMode")
                lfCmd("autocmd!")
    ...
    def _search(self, content, is_continue=False, step=0):
        if "--live" not in self._arguments:
            super(GitExplManager, self)._search(content, is_continue, step)
            return
